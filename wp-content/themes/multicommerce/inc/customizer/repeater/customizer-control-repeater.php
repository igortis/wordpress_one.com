 ...
 */
if ( ! function_exists( 'multicommerce_sanitize_social_data' ) ) :
	function multicommerce_sanitize_social_data( $input ){
		...
		if( !empty( $input_decoded ) ) {
			foreach ( $input_decoded as $boxes => $box ){
				foreach ( $box as $key => $value ){
                    ...
                    elseif ( $key == 'checkbox' ){
						$input_decoded[$boxes][$key] = multicommerce_sanitize_checkbox( $value );
					}
					else{
...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Multicommerce_Customize_Icons_Control' )):
	/**
	...
	class Multicommerce_Customize_Icons_Control extends WP_Customize_Control {
            ...
            <label>
                ...
                <span class="te-customize-icons">
                    ...
                    <span class="icon-toggle">
                        <?php echo ( empty( $value ) ? esc_html__('Add Icon','multicommerce'): esc_html__('Change Icon','multicommerce') ); ?>
                        <span class="dashicons dashicons-arrow-down"></span>
                    </span>
                    <span class="icons-list-wrapper hidden">
                        <input class="icon-search" type="text" placeholder="<?php esc_attr_e('Search Icon','multicommerce'); ?>">
	                    <?php
...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Multicommerce_Repeater_Control' )):
	/**
	 ...
	 *
	 */
	class Multicommerce_Repeater_Control extends WP_Customize_Control {
		/**
		 * The control type.
		 *
		 * @access public
		...
		public function enqueue(){
			...
			wp_enqueue_style( 'multicommerce-customizer', get_template_directory_uri() . '/assets/css/customizer.min.css', array(), '1.0.2' );
			wp_style_add_data( 'multicommerce-customizer', 'rtl', 'replace' );
		}
		...
		public function render_content() {
			...
			?>
            <ul class="te-repeater-field-control-wrap">
				<?php $this->get_fields(); ?>
            </ul>
            <input type="hidden" <?php $this->link(); ?> class="te-repeater-collection" value="<?php echo esc_attr( $this->value() ); ?>"/>
            <button type="button" class="button te-repeater-add-control-field"><?php echo esc_html( $this->repeater_add_control_field ); ?></button>
			<?php
		}
		...
		private function get_fields() {
			...
			?>
            <script type="text/html" class="te-repeater-field-control-generator">
                    ...
                    <div class="repeater-fields hidden">
								...
								switch ( $field['type'] ) {
									...
									case 'icons':
                                        ...
                                        <span class="te-customize-icons">
                                            ...
                                            <span class="icon-toggle">
                                                <?php esc_html_e('Add Icon','multicommerce'); ?>
                                                <span class="dashicons dashicons-arrow-down"></span>
                                            </span>
                                            <span class="icons-list-wrapper hidden">
                                                <input class="icon-search" type="text" placeholder="<?php esc_attr_e('Search Icon','multicommerce'); ?>">
                        ...
                        <div class="clearfix repeater-footer">
                            <a class="repeater-field-remove" href="#remove">
								<?php esc_html_e( 'Delete', 'multicommerce' ) ?>
                            </a>
                            <?php esc_html_e( '|', 'multicommerce' ) ?>
                            <a class="repeater-field-close" href="#close">
								<?php esc_html_e( 'Close', 'multicommerce' ) ?>
                            </a>
                        </div>
                    </div>
                </li>
                    ...
                    <li class="repeater-field-control">
                        ...
                        <div class="repeater-fields hidden">
									...
									switch ( $field['type'] ) {
										...
										case 'icons':
                                            ...
                                            <span class="te-customize-icons">
                                                ...
                                                <span class="icon-toggle">
                                                    <?php echo ( empty( $new_value )? esc_html__('Add Icon','multicommerce'): esc_html__('Edit Icon','multicommerce') ); ?>
                                                    <span class="dashicons dashicons-arrow-down"></span>
                                                </span>
                                                <span class="icons-list-wrapper hidden">
                                                    <input class="icon-search" type="text" placeholder="<?php esc_attr_e('Search Icon','multicommerce')?>">
                            ...
                            <div class="clearfix repeater-footer">
                                <a class="repeater-field-remove" href="#remove">
									<?php esc_html_e( 'Delete', 'multicommerce' ) ?>
                                </a><?php esc_html_e( '|', 'multicommerce' ) ?>
                                <a class="repeater-field-close" href="#close">
									<?php esc_html_e( 'Close', 'multicommerce' ) ?>
                                </a>
                            </div>
