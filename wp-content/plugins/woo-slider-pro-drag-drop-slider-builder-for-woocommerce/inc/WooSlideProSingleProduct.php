...
global $wishlists;
class WooSlideProSingleProduct
{
    ...
    private function getStarRating($score)
    {
        ...
        for ($i = 0; $i < $whole; $i++)
        {
            $content.= '<i class="fa fa-star full-star"></i>';
        }
        if ($fract == 0.5)
            $content.= '<i class="fa fa-star-half-full half-star"></i>';
        else if ($fract > 0.5)
            $content.= '<i class="fa fa-star full-star"></i>';
        else if ($fract > 0 && $fract < 0.5)
        {
            $content.= '<i class="fa fa-star-o empty-star"></i>';
        }
...
//                )). '</div>';
            ...
            case '[[add_to_cart_button]]':
                ...
                $content = '<div class="woo-slider-pro-add-to-cart-button"> <button class="wsp-add-to-cart" data-url="'.$this->product->add_to_cart_url() . '"><span class="wsp-add-to-cart-text">'.$this->product->add_to_cart_text().'</span></button> </div>';
                break;
            default:
                $content = '';
                break;
